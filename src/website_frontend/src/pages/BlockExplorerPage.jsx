  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        if (Block) {
          const trxblock = trx.find((tx) => String(tx.blockIndex) === Block);
          setTransaction(trxblock);
          return;
        }
        ...
        {block || hash ? (
          <Box className="">
            ...
            <div className="w-full flex flex-col justify-center items-center gap-2">
              <div
                className="w-full p-2 text-sm bg-secondaryColor border border-borderShade/35 rounded-xl"
              >
          ...
          <>
            <Box className="w-full grid grid-cols-3 gap-2 border-2 border-black">
            ...
            <div className="w-full flex gap-4 text-white">
              <Box className="w-[30%]">
                <div className="w-full h-16 flex justify-start  items-center">
                  <h2 className="font-semibold">Latest Block</h2>
                </div>
                <div className="w-full flex flex-col justify-center items-center gap-2">
                  {transactions.map((block, index) => (
                    <div
                      key={index}
                      className="w-full flex justify-evenly p-2 items-center text-sm gap-2 bg-secondaryColor border border-borderShade/35 rounded-xl"
                    >
                      <div className="w-[30%] flex items-center">
                        ...
                        <div className="ms-5 cursor-pointer text-blue-600" onClick={() => navigate(`/dyascan?block=${block.blockIndex}`)}>{block.blockIndex}</div>
                      </div>
                      <div className="w-[70%] flex flex-col items-start justify-center">
                        <span className="text-gray-600">{timeAgo(block.timestamp)}</span>
                      </div>
                    </div>
                  ))}
                </div>
              ...
              <Box className="w-[70%]">
                ...
                <div className="w-full flex flex-col justify-center items-center gap-2">
                  {transactions.map((tx, index) => (
                    <div
                      key={index}
                      className="w-full flex justify-between p-4 text-sm items-center bg-secondaryColor border border-borderShade/35 rounded-xl"
                    >
                      <div className="w-[10%]">
                        <svg
                          xmlns="http://www.w3.org/2000/svg"
                      ...
                      <div className="w-[40%]">
                        <p>
                          From:{" "}
                          <span className="text-gray-600">{shorten(tx.from)}</span>
                        </p>
                        <p>
                          To:{" "}
                          <span className="text-gray-600">{shorten(tx.to)}</span>
                        </p>
                      </div>
